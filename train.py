...
class Instructor:
    ...
    def _train(self, criterion, optimizer, train_data_loader, val_data_loader,classifier_data_loader=None):
        ...
        if self.opt.gating:
            for epoch in range(self.opt.num_epoch):
                ...
                for i_batch, sample_batched in enumerate(train_data_loader):
                    ...
                    outputs,cls_asp = self.model.gating(inputs)
                    targets = sample_batched['aspect_category'].to(self.opt.device)
        ...
        if self.opt.classifier:
            for epoch in range(self.opt.num_epoch):
                ...
                # switch model to training mode
                self.model.train()
                for i_batch, sample_batched in enumerate(classifier_data_loader):
                    global_step += 1
                    # clear gradient accumulators
                    optimizer.zero_grad()
                    inputs = [sample_batched[col].to(self.opt.device) for col in self.opt.inputs_cols]
                    outputs,cls_asp = self.model.classifier(inputs)
                    ...
                    if global_step % self.opt.log_step == 0:
                        ...
                        train_loss = loss_total / n_total
                        logger.info('loss: {:.4f}, acc: {:.4f}'.format(train_loss, train_acc))
            print('pretrain classifier down')
    ...
    def _evaluate_acc_f1(self, data_loader):
        ...
        #     test_pd = pd.read_csv(open('dataset_pd/Restaurants_Test_Gold.xml.seg.pd','r'))
        # elif self.opt.dataset == 'laptop':
        #     test_pd = pd.read_csv(open('dataset_pd/Laptops_Test_Gold.xml.seg.pd','r'))
        # else:
        #     test_pd = pd.read_csv(open('dataset_pd/test.raw.pd','r'))
        
        # true = t_targets_all.cpu()
        # preidc = torch.argmax(t_outputs_all, -1).cpu()
        # wrong = (true != preidc)
        
        # wrong_pd = test_pd[wrong.cpu().numpy()>0.5]
        # path = 'state_dict/{0}_{1}_val_acc_err'.format(self.opt.model_name, self.opt.dataset)
        # wrong_pd.to_csv(open(path,'w'),index=False)
        acc = n_correct / n_total
        target_names = ['class 0', 'class 1', 'class 2']
        
        f1 = metrics.f1_score(t_targets_all.cpu(), torch.argmax(t_outputs_all, -1).cpu(), average='macro')
        print(classification_report(t_targets_all.cpu(), torch.argmax(t_outputs_all, -1).cpu(), target_names=target_names))
        return acc, f1
...
def main():
    ...
    dataset_files = {
        ...
        'restaurant': {
            ...
            'train': './datasets/semeval14/Restaurants_Train_aspect.xml.seg',
            'test': './datasets/semeval14/Restaurants_Test_Gold_aspect.xml.seg'
        },
        'laptop': {
    ...
    input_colses = {
        ...
        'td_lstm': ['text_left_with_aspect_indices', 'text_right_with_aspect_indices','aspect_indices'],
        ...
        'bert_spc': ['text_bert_indices', 'bert_segments_ids','text_bert_mask'],
        'aen_bert': ['text_raw_bert_indices', 'aspect_bert_indices'],
        'aen_glove':['text_raw_bert_indices', 'aspect_bert_indices'],
    }
    initializers = {
